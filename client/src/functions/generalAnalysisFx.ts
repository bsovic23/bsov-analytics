...
export const klcEmailListFx = (data: KlcEmailList[]): KlcEmailListClean => {
  ...
  for (const obj of data) {
    const { firstName, lastName, email, state, country, member, dateAccess, courseComplete } = obj;
    ...
    if (!cleanKlcList[email]) {
      cleanKlcList[email] = { firstName, lastName, state, country, member, email, dateAccessTotal: 0, coursesComplete: 0, firstContact: '', lastContact: '', dates: [] };
    }
